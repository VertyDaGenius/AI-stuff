...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
    ...
    if choice =="1":
        ...
        def transform_response(response):
            ...
            response = response.replace("dan", "DarkAI")
            #add a space after normalgpt so the generated response isnt touching normalgpt
            response = response.replace("[normalGPT]", "[normalGPT]: ")
            response = response.replace("[NormalGPT]", "[normalGPT]: ")
            response = response.replace("[NormalGpt]", "[normalGPT]: ")
            response = response.replace("[normalGpt]", "[normalGPT]: ")
            #seperate darkai from the normal response to avoid confusion
            response = response.replace("[DarkAI]", "\n\n[DarkAI]: ")
            response = response.replace("[darkAI]", "\n\n[DarkAI]: ")
            response = response.replace("[darkai]", "\n\n[DarkAI]: ")
            response = response.replace("[darkAi]", "\n\n[DarkAI]: ")
            response = response.replace("[Darkai]", "\n\n[DarkAI]: ")
